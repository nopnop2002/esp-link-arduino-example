...
void setup() {
  ...
  // Set up the REST client to talk to worldclockapi.com, this point doesn't connect to that server,
  // it just sets-up stuff on the esp-link side
  // http://zipcloud.ibsnet.co.jp/api/search?zipcode=7830060

  int err = rest.begin("zipcloud.ibsnet.co.jp");
  if (err != 0) {
...
void loop() {
  ...
  if(wifiConnected) {
    // Request to the previously set-up server
    rest.get("/api/search?zipcode=7830060");
